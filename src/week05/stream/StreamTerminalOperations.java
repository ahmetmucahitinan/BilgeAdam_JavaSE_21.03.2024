...
public class StreamTerminalOperations {
    public static void main(String[] args) {
        ...
        String[] stringArr = "Merhaba Dünya Ben Ahmet. Nasılsın?".split(" ");
        ...
        // Stream boşsa true değerini döndürür.
        // Sonsuz Streamler için sonlandırılabilir, sonlandırılmayabilir.(verilere bağlıdır)
        System.out.println(list.stream().noneMatch(pred)); // false döndü

        // infinitive
        System.out.println(infinite01.anyMatch(pred)); // true döndü
//        System.out.println(infinite01.allMatch(pred)); //true döndü
...
//        s01.forEach(System.out::println);
        ...
        /*  Bu ekrana yazdırdığımız kısmı ayrı işlem satırlarında görmek istediğimizde yapacağımız yoldur.
          List<Integer> mappedNumbers = numbers.stream()
                .map(number -> number * 2)
                .map(number -> number * 2)
                .collect(Collectors.toList())
...
//            String word = stream03.reduce("X",(x,y) -> x.concat(y));
        ...
        // Veriler aynı olsada olmasada verilerin tamamını yazar ve ayrıca sırasına göre yazar
        List<String> gmailList = Arrays.stream(Pattern.compile(":")
                .split(frenchCounting))
                .collect(Collectors.toList());
        ...
        // Veriler den aynıları tekrar ediyorsa tek bir veri yazar.
        Set<String> gmailSet = Pattern.compile(":")
                .splitAsStream(frenchCounting).collect(Collectors.toSet());
        System.out.println(gmailSet);

       // collect(Collectors.toMap())
        Map<String, Integer> nameLength = Stream.of("Elijah", "Anuoluwapo", "Oluwaferanmi", "Oluwatomi", "Oluwalana", "Ayomikulehin", "Adebayo")
                .collect(Collectors.toMap(name -> name, String::length));
        System.out.println(nameLength);

        Optional<String> name = Stream.of("Elijah", "Anuoluwapo", "Oluwaferanmi", "Oluwatomi", "Oluwalana", "Ayomikulehin", "Adebayo")
